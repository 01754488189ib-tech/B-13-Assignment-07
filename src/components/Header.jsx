...
const Header = () => {
    ...
    const navItems = [
        { name: 'Home', link: '#', icon: <TiHomeOutline size={18} /> },
        { name: 'Timeline', link: '#', icon: <RiTimeLine size={18} /> },
        { name: 'Stats', link: '#', icon: <MdOutlineQueryStats size={18} /> }
    ];
                    ...
                    <ul tabIndex={0} className="menu menu-sm dropdown-content mt-3 p-2 shadow bg-base-100 rounded-box w-52 z-[50]">
                        {navItems.map((item, index) => (
                            <li key={index} onClick={() => setActiveTab(item.name)}>
                                <a className={`flex items-center gap-2 ${activeTab === item.name ? "text-[#244D3F] font-bold" : ""}`}>
                                    {item.icon}
                                    {item.name}
                                </a>
                            </li>
